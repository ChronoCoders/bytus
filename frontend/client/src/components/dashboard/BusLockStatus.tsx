...
interface BusLockStatusProps {
  lockedAmount: number;
  requiredAmount: number;
  processingVolume: number;
}

export function BusLockStatus({ lockedAmount, requiredAmount, processingVolume }: BusLockStatusProps) {
  ...
  return (
    <Card className="shadow-xs hover:shadow-md transition-shadow">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm font-medium text-muted-foreground">BUS Lock Status</CardTitle>
      ...
      <CardContent>
        ...
        <div className="flex items-center justify-between text-xs">
          <span className={isHealthy ? "text-green-600 font-medium" : "text-yellow-600 font-medium"}>
            {isHealthy ? "Requirements Met" : "Action Required"}
          </span>
          <span className="text-muted-foreground">0.1% of Volume</span>
        </div>
        ...
        {!isHealthy && (
          <div className="mt-3 bg-yellow-50 dark:bg-yellow-900/20 p-2 rounded text-xs text-yellow-800 dark:text-yellow-200 flex items-start gap-2">
            ...
            <span>Increase lock to support current processing volume of ${processingVolume.toLocaleString()}.</span>
          </div>
        )}
      </CardContent>
    </Card>
